	...
	$.each(objJson, function(key, value) {
			if (objJson[ligne].resource.id==FreebusyRoom.ID) {
				...
				if (compareTime(end,now)) {					
					...
					if (objJson[ligne].presenceConfirmedDate!="") presenceConf=true;
					resList[ligne]=[start,end,owner,ownerPhone,subject, order, presenceConf]
				}
			}
		ligne++;
	});
...
function refresh() {//rafraichissement de l'�cran!
...
function endAbooking(){//permet d'�courter une resa.
