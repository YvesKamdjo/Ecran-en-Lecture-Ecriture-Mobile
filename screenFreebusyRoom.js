...
function initDocument(){
	//initialisations des variables globales et mise en forme de la page en fonction de l'écran
	...
	getUrbaToken(getFreeRoomList);// on veut d'abord savoir si la salle est considérée comme libre ou occupée par l'API
	$(window).resize(function(){
	afficherHeureSurFrise();
	});
}
...
function checkRoomVacancy(objJson) {//vérifie si la salle appartient à la liste des salles libres de la prochaine demi-heure
	var i=0;
	var now=getTime();
...
function fillRoomInfo(objJson){// récupère les informations basiques de la salle : nom, heures pendant lesquelles il est possible de réserver
...
function fillResListforRoom(objJson) {// récupère les infos des réservations pour les afficher
	var ligne=0;
	var j=0;
	var jsonLocal=[];
	var resList=[];
...
function button_res() {// le bouton montre ou cache le menu pour envoyer une réservation
...
function res_demand(minutes) {//envoie une réservation sur une durée choisie par l'utilisateur
